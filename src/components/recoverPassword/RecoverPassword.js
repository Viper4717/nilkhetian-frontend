...
function changeUserPass(passwordObject, setLoading, setError){
    Axios.post(`${serverUrl}/api/user/resendconfirmation`, passwordObject)
    .then(({data: res}) => {
        window.location.assign('/recoverpassword/success');
    })
    .catch((error) => {
        window.scrollTo(0, 0);
    ...
    function changePass(e){
        ...
        if(password && confirmPass){
            ...
            if(password.length < 8 || password !== confirmPass){
                ...
                changeUserPass(passwordObject, setLoading, setError);
            }
        }
        else{
            window.scrollTo(0, 0);
            const err = "Please fill out both fields."
            setError(err);
        }
    }
